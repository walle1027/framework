 ...
 */
package org.loed.framework.jdbc.database.model;

/**
 * @author Richard This class contains a list of all the tables for which
 * foreign keys exist for the containing SqlTable. It contains a
 * reference to the parent and also a Hashtable of foreign keys for each
 * table
 */
public class ForeignKeys {
	...
	public void addForeignKey(String tableName, String columnName,
	                          String parentColumn, Integer seq) {
		ForeignKey tbl = null;
	...
	public boolean getHasImportedKeyColumn(String aColumn) {
		...
		for (int i = 0; i < numKeys; i++) {
			ForeignKey aKey = (ForeignKey) getAssociatedTables()
					.getOrderedValue(i);
